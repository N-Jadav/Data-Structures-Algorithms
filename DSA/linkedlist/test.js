...
describe('Reverse', () => {
  test('reverses an empty linked list', () => {
    const l = new List();
    l.reverse();
    expect(l.head).toBeNull();
  });

  test('reverses a linked list with one node', () => {
    const l = new List();
    l.insertLast('a');
    l.reverse();
    expect(l.head.data).toEqual('a');
  ...
  test('reverses a linked list with multiple nodes', () => {
    const l = new List();
    l.insertLast('a');
    l.insertLast('b');
    l.insertLast('c');
    l.reverse();
    expect(l.getAt(0).data).toEqual('c');
    expect(l.getAt(1).data).toEqual('b');
    expect(l.getAt(2).data).toEqual('a');
    expect(l.getAt(2).next).toBeNull();
  });
});
